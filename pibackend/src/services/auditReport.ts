...
function buildToolCalls(result: Omit<AuditRunResult, "explainability">): AuditExplainability["toolCalls"] {
  ...
  const calls: AuditExplainability["toolCalls"] = [
    ...
    {
      order: order++,
      tool: "vultr.vector_search.flash",
  ...
  for (const retrieval of result.retrievals) {
    calls.push({
      order: order++,
      tool: "vultr.vector_search.prime",
  ...
  for (const check of result.reflectiveChecks) {
    calls.push({
      order: order++,
      tool: "vultr.reflective_retrieval",
...
function buildCaveats(result: Omit<AuditRunResult, "explainability">): string[] {
  ...
  if (result.rulebook.agreementName.includes("placeholder")) caveats.push("Covenant rule extraction used the fallback rulebook; strengthen rule extraction before production use.");
